...
describe("مسار تشغيل Vercel", () => {
  it("لا يستخدم مدخل TypeScript يُترجم إلى استيراد بدون امتداد", () => {
    ...
    expect(readFileSync(join(process.cwd(), "api/index.ts"), "utf8").trim()).toBe('export { default } from "./handler-entry.js";');
    expect(existsSync(join(process.cwd(), "api/handler-entry.ts"))).toBe(true);
    ...
    const handlerEntry = readFileSync(join(process.cwd(), "api/handler-entry.ts"), "utf8");
