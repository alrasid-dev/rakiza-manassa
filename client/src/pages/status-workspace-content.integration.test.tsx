...
vi.mock("@/lib/trpc", () => ({ trpc: { useUtils: () => ({ court: { attendance: { list: { invalidate: vi.fn() } }, leave: { list: { invalidate: vi.fn() } } } }), court: { registration: { myPermission: { useQuery: () => ({ data: "employee" }) } }, myRoles: { useQuery: () => ({ data: ["court_president"] }) }, units: { list: { useQuery: () => ({ data: [{ id: 44, name: "قسم عن بعد" }], isLoading: false }) } }, people: { list: { useQuery: () => ({ data: people }) } }, achievements: { mine: { useQuery: () => ({ data: { summary: { positive: 8, negative: 2, balance: 6 } }, error: null }) } }, disciplinary: { mine: { useQuery: () => ({ data: [{ approval: { id: 5, status: "pending", requestNote: "متابعة آلية" }, taskTitle: "مهمة مختبرة" }], error: null }) } }, attendance: { confirmationConfig: { useQuery: () => ({ data: { isActive: false, cronExpression: "0 0 4-12 * * 0-4", targetUnitIds: [], audience: "all", shiftEnabled: false }, isLoading: false }) }, updateConfirmationConfig: { useMutation: () => ({ mutate: vi.fn(), isPending: false, error: null }) }, self: { useQuery: () => ({ data: people[0] }) }, serverClock: { useQuery: () => ({ data: { now: new Date("2026-08-14T07:00:00Z") } }) }, list: { useQuery: () => ({ data: [] }) }, remoteReport: { useQuery: () => ({ data: [{ attendance: { id: 91, recordDate: new Date("2026-08-14T07:00:00Z"), status: "present" }, profileName: "موظف عن بعد", attendanceMode: "remote", unitId: 44 }], error: null }) }, record: { useMutation: () => ({ mutate: vi.fn(), isPending: false, error: null }) }, checkout: { useMutation: () => ({ mutate: vi.fn(), isPending: false, error: null }) } }, leave: { list: { useQuery: () => ({ data: [] }) }, substitutes: { useQuery: () => ({ data: [{ id: 4, fullName: "بديل مختبر" }], isLoading: false, error: null }) }, submit: { useMutation: () => ({ mutate: vi.fn(), isPending: false, error: null }) } } } } }));
