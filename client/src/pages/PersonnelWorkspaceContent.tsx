...
import ProfileAssignmentPanel from "@/components/ProfileAssignmentPanel";
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import { Input } from "@/components/ui/input";
import { trpc } from "@/lib/trpc";
import { ArrowLeftRight, ChevronDown, CircleDashed, ClipboardCheck, FilePenLine, FilePlus2, Handshake, Search, UsersRound } from "lucide-react";
...
export default function PersonnelWorkspaceContent() {
  ...
  const openTransfer = (profile: Profile) => {
    setTransferTarget(profile);
    setTransferUnitId(profile.unitId ? String(profile.unitId) : "");
    setTransferReason("");
  };
  const submitTransfer = (event: FormEvent) => {
    ...
    update.mutate({
      profileId: transferTarget.id,
      unitId: Number(transferUnitId),
      directManagerProfileId: transferTarget.directManagerProfileId ?? null,
      fullName: transferTarget.fullName,
      email: transferTarget.email || undefined,
      employeeNumber: transferTarget.employeeNumber || undefined,
      jobTitle: transferTarget.jobTitle || undefined,
      judicialFormation: transferTarget.judicialFormation || undefined,
      attendanceMode: transferTarget.attendanceMode || "in_person",
      status: transferTarget.status,
      reason: transferReason || `نقل تشغيلي إلى قسم رقم ${transferUnitId}`,
    }, {
      onSuccess: async () => {
        await utils.court.people.list.invalidate();
        setTransferTarget(null);
        toast.success("تم تسجيل النقل وتحديث القسم.");
      },
    });
  };
  const focusAssignment = () => {
    setAssignmentFocus(true);
    setLocation("/people#hr-assignment");
    requestAnimationFrame(() => document.getElementById("hr-assignment")?.scrollIntoView({ behavior: "smooth", block: "start" }));
  };
  useEffect(() => {
    if (typeof window === "undefined") return;
    if (window.location.hash === "#hr-assignment") {
      setAssignmentFocus(true);
      requestAnimationFrame(() => document.getElementById("hr-assignment")?.scrollIntoView({ behavior: "smooth", block: "start" }));
    }
  }, []);
  ...
  return <section className="mx-auto max-w-6xl">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold tracking-[0.14em] text-[#b18448]">الموظفون الإداريون والملازمون</p><h1 className="mt-2 text-3xl font-bold text-[#12352f]">الأفراد والتشكيلات</h1><p className="mt-2 max-w-2xl text-sm leading-7 text-[#65766d]">إدارة الملفات حسب القسم، مع أيقونات النقل والتكليف والتفويض المربوطة بمسارات حقيقية.</p></div><div className="grid h-12 w-12 place-items-center rounded-2xl bg-[#e9f0ea] text-[#1f5a47]"><UsersRound className="h-6 w-6" /></div></div>
    <div className="mt-5 grid gap-3 sm:grid-cols-3" role="group" aria-label="إجراءات الموارد البشرية">
      <button type="button" disabled={!canManage} onClick={() => { const first = filteredPeople[0]; if (first) openTransfer(first); else toast.message("اختر موظفاً من القائمة ثم اضغط أيقونة النقل بجانبه."); }} className="flex items-center gap-3 rounded-2xl border border-[#d8e6dc] bg-[#f7fbf7] px-4 py-3 text-right shadow-sm disabled:opacity-50">
        <span className="grid h-10 w-10 place-items-center rounded-xl bg-[#e3eee2] text-[#2d6b4f]"><ArrowLeftRight className="h-5 w-5" /></span>
        <span><span className="block text-sm font-black text-[#12352f]">نقل</span><span className="mt-0.5 block text-[11px] text-[#65766d]">تغيير القسم عبر تحديث الملف</span></span>
      </button>
      <button type="button" disabled={!canManage} onClick={focusAssignment} className="flex items-center gap-3 rounded-2xl border border-[#d8e6dc] bg-[#f7fbf7] px-4 py-3 text-right shadow-sm disabled:opacity-50">
        <span className="grid h-10 w-10 place-items-center rounded-xl bg-[#efe4c0] text-[#80642b]"><ClipboardCheck className="h-5 w-5" /></span>
        <span><span className="block text-sm font-black text-[#12352f]">تكليف</span><span className="mt-0.5 block text-[11px] text-[#65766d]">سجل تكليف تشغيلي مستقل</span></span>
      </button>
      <Link href="/delegation" className="flex items-center gap-3 rounded-2xl border border-[#d8e6dc] bg-[#f7fbf7] px-4 py-3 text-right shadow-sm">
        <span className="grid h-10 w-10 place-items-center rounded-xl bg-[#e8eef8] text-[#3d5a80]"><Handshake className="h-5 w-5" /></span>
        <span><span className="block text-sm font-black text-[#12352f]">تفويض</span><span className="mt-0.5 block text-[11px] text-[#65766d]">تفويض صلاحية مؤقت من القيادة</span></span>
      </Link>
    </div>
