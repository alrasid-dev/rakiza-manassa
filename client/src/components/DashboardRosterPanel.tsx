...
export default function DashboardRosterPanel() {
  ...
  const canBrowse = permission.data === "full_control" || permission.data === "general_view" || (roles.data ?? []).some(role => ["court_president", "assistant_president", "court_secretary", "human_resources_manager", "department_manager"].includes(role));
  const peopleQuery = trpc.court.people.list.useQuery(undefined, { enabled: canBrowse });
  const unitsQuery = trpc.court.units.list.useQuery(undefined, { enabled: canBrowse });
