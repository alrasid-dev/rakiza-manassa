...
- رابط التشغيل الحي المجاني (Vercel + HTTPS): https://rakiza-manassa.vercel.app
...
- الرابط الآمن الحالي: https://rakiza-manassa.vercel.app — النطاق الفرعي المجاني مع شهادة HTTPS تلقائية مجددة تلقائياً.
- فحص الجهوزية: https://rakiza-manassa.vercel.app/health (يُظهر جهوزية قاعدة البيانات وSupabase دون كشف أي سر).
...
| كلمة المرور | تُعيَّن أول مرة من 8 خانات تضم حرفاً ورقماً، ثم يستمر المستخدم عليها دون تحقق إضافي في كل دخول |
...
قواعد السياسة في الشيفرة: `shared/password-policy.ts` لقاعدة كلمة المرور، و`server/court-service.ts` (`isAllowedLoginEmail` و`getNotificationEmailRecipients`) للبريد وقنوات التنبيه.
